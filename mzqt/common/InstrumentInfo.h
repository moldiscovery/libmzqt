...
namespace mzqt {
    ...
    public:
        ...
        QString instrumentName_; // "Machine #7", etc
        QString instrumentSerialNumber_;
        QString instrumentHardwareVersion_;

        MSAcquisitionSoftwareType acquisitionSoftware_; // xcalibur, masslynx, masshunter, etc
        QString acquisitionSoftwareVersion_;

        InstrumentInfo(void) :
            manufacturer_(MANUFACTURER_UNDEF), instrumentModel_(
                    INSTRUMENTMODEL_UNDEF), ionSource_(IONIZATION_UNDEF),
                    detector_(DETECTOR_UNDEF), acquisitionSoftware_(
                            ACQUISITIONSOFTWARE_UNDEF)
        {
        }
    };
