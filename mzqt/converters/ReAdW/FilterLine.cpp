...
MSAnalyzerType FilterLine::parseAnalyzer(const string& word)
{
    if (word == "ITMS") {
        return ITMS;
    }
...
MSPolarityType FilterLine::parsePolarity(const string& word)
{
    if (word == "+") {
        return POSITIVE;
    }
...
MSScanDataType FilterLine::parseScanData(const string& word)
{
    if (word == "C") {
        return CENTROID;
    }
...
MSIonizationType FilterLine::parseIonizationMode(const string & word)
{
    if (word == "EI") {
        return EI;
    }
...
FilterLine::AccurateMassType FilterLine::parseAccuracteMass(const string& word)
{
    if (word == "!AM") {
        return NO_AM;
    }
...
MSScanType FilterLine::parseScanType(const string& word)
{
    if (word == "FULL") {
        return FULL;
    }
...
MSActivationType FilterLine::parseActivation(const string& word)
{
    if (word == "CID") {
        return CID;
    }
...
bool FilterLine::parse(string filterLine)
{
    ...
    // Enhanced mode
    // NOTE: unrecorded, parse only;

    // my Thermo information had this as magnetic sector only, but
    ...
    if (scanType_) {
        ...
        // while we have the scanType, let's go ahead and set msLevel for the scans types that we can,
        // in case there is no "MS order" indicator ahead
        if (scanType_ == Q1MS) {
            msLevel_ = 1;
        }
    }
    ...
    if (msLevel_ > 1) {
        ...
        for (int i = 0; i < expectedPairs; ++i) {
            ...
            if ((markerPos == string::npos)) {
                // no "@" found.  Assume that this is a SRM scan of form
                // "mass [range1,range2]
                markerPos = w.find_first_not_of("0123456789.");
                string mass = w.substr(0, markerPos);
                cidParentMass_.push_back(toDouble(mass));
                cidEnergy_.push_back(0); // not included in filter line, store as 0
                // prematurely done?
                if (s.eof()) {
                    return false;
                }
            ...
            // make sure this word starts with a numeric char (the word contains "@"):
            else if (!((c >= '0') && (c <= '9'))) {
                // return error
                return false;
            }
            ...
            if (energy[0] >= '0' && energy[0] <= '9') {
                ...
                string::size_type numericPos = energy.find_first_of(
                        "0123456789");
